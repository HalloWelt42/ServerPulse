    ...
    private var emptyState: some View {
        VStack(spacing: 14) {
            Image(systemName: "server.rack")
                .font(.system(size: AppTheme.scaled(44)))
...
struct UnifiedServerCard: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                Text(server.name)
                    .font(.system(size: AppTheme.scaled(15), weight: .bold))
                    .foregroundStyle(AppTheme.textPrimary)
                    .lineLimit(1)
                    .truncationMode(.tail)
                ...
                if showTemperatureInCard, let temp = metrics?.temperature?.maxTemperature {
                    HStack(spacing: 3) {
                        ...
                        Text("\(Int(temp))°C")
                            .font(.system(size: AppTheme.scaled(12), weight: .semibold, design: .monospaced))
                            .foregroundStyle(tempColor(temp))
                    }
                }
                    ...
                    Button { onTap() } label: {
                        HStack(spacing: 4) {
                            Image(systemName: "chart.bar.fill")
                                .font(.system(size: AppTheme.scaled(9)))
                        ...
                        .padding(.vertical, 5)
                        .background(isHoveringDetails ? AppTheme.buttonPrimaryHover : AppTheme.buttonPrimary)
                        .clipShape(Capsule())
                    }
                    .buttonStyle(.plain)
                ...
                Button {
                    ...
                    Image(systemName: "trash")
                        .font(.system(size: AppTheme.scaled(11)))
                        .foregroundStyle(isHoveringDelete ? AppTheme.buttonDanger : AppTheme.textTertiary)
                        .padding(4)
                        .background(isHoveringDelete ? AppTheme.buttonDangerBg : Color.clear)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 10)
                .stroke(isHovering ? AppTheme.buttonPrimary.opacity(0.4) : (isConnected ? AppTheme.border : AppTheme.border.opacity(0.5)), lineWidth: 1)
        )
    ...
    private var connectToggle: some View {
        Button {
            ...
            HStack(spacing: 5) {
                ...
                if connectionState == .connecting {
                    ...
                    Text(isConnected ? loc["dashboard.card.on"] : loc["dashboard.card.off"])
                        .font(.system(size: AppTheme.scaled(10), weight: .bold))
                        .foregroundStyle(isConnected ? AppTheme.textPrimary : AppTheme.textTertiary)
                        .lineLimit(1)
                }
            }
            .fixedSize()
            ...
            .background(
                isHoveringConnect
                    ? (isConnected ? AppTheme.buttonPrimary.opacity(0.3) : AppTheme.buttonSecondary.opacity(0.5))
                    : (isConnected ? AppTheme.buttonPrimary.opacity(0.15) : AppTheme.border.opacity(0.5))
            )
            .clipShape(Capsule())
        }
        .buttonStyle(.plain)
    ...
    private var statusColor: Color {
        ...
        case .connected: return AppTheme.statusOnline
        case .connecting: return AppTheme.statusWarning
        case .error: return AppTheme.statusOffline
        ...
        HStack(spacing: 14) {
            ...
            VStack(spacing: 2) {
                GaugeRing(
                    ...
                    color: AppTheme.utilizationColor(m.memory.usagePercent),
                    lineWidth: 4,
                    size: 42,
                    fontSize: 11,
                    animateOnTap: true
            ...
            VStack(alignment: .trailing, spacing: 3) {
                ...
                HStack(spacing: 4) {
                    Image(systemName: "arrow.up")
                        ...
                        .foregroundStyle(AppTheme.activityUploadColor(totalTx))
                    Text(ByteFormatter.formatRate(totalTx))
                        .font(.system(size: AppTheme.scaled(10), weight: .bold, design: .monospaced))
                        .foregroundStyle(totalTx > 0 ? AppTheme.textPrimary : AppTheme.textTertiary)
                }
                HStack(spacing: 4) {
                    Image(systemName: "arrow.down")
                        .font(.system(size: AppTheme.scaled(8), weight: .bold))
                        .foregroundStyle(AppTheme.activityDownloadColor(totalRx))
                    Text(ByteFormatter.formatRate(totalRx))
                        .font(.system(size: AppTheme.scaled(10), weight: .bold, design: .monospaced))
                        .foregroundStyle(totalRx > 0 ? AppTheme.textPrimary : AppTheme.textTertiary)
                }
            }
            .fixedSize()
            ...
            VStack(alignment: .trailing, spacing: 3) {
                ...
                HStack(spacing: 4) {
                    Text("R")
                        .font(.system(size: AppTheme.scaled(9), weight: .bold))
                        .foregroundStyle(AppTheme.activityReadColor(totalRead))
                    Text(ByteFormatter.formatRate(totalRead))
                        .font(.system(size: AppTheme.scaled(10), weight: .bold, design: .monospaced))
                        .foregroundStyle(totalRead > 0 ? AppTheme.textPrimary : AppTheme.textTertiary)
                }
                HStack(spacing: 4) {
                    Text("W")
                        .font(.system(size: AppTheme.scaled(9), weight: .bold))
                        .foregroundStyle(AppTheme.activityWriteColor(totalWrite))
                    Text(ByteFormatter.formatRate(totalWrite))
                        .font(.system(size: AppTheme.scaled(10), weight: .bold, design: .monospaced))
                        .foregroundStyle(totalWrite > 0 ? AppTheme.textPrimary : AppTheme.textTertiary)
                }
            }
            .fixedSize()
        }
    ...
    private func tempColor(_ temp: Double) -> Color {
        if temp >= 80 { return AppTheme.statusOffline }
        if temp >= 60 { return AppTheme.statusWarning }
            ...
            ForEach(Array(mainDisks)) { disk in
                HStack(spacing: 6) {
                    Text(disk.mountPoint)
                        .font(.system(size: AppTheme.scaled(9), design: .monospaced))
                        .foregroundStyle(AppTheme.textTertiary)
                        .lineLimit(1)
                        .truncationMode(.middle)
                    ...
                    Text("\(Int(disk.usagePercent * 100))%")
                        .font(.system(size: AppTheme.scaled(9), weight: .bold, design: .monospaced))
                        .foregroundStyle(AppTheme.utilizationColor(disk.usagePercent))
                        .lineLimit(1)
                        .fixedSize()
                }
            }
